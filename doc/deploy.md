...
| `scripts/deploy-remote.sh` | 服务端 | 接收产物 → 校验 → 维护页 → 替换 → 启动 → 健康检查 |
| `scripts/webhook-server.js` | 服务端 | 监听 GitHub/Gitee 推送，分支推送自动 git pull |
| `scripts/maintenance-server.js` | 服务端 | 部署期间显示维护页面 |
...
Phase 4  维护页
  └─ node maintenance-server.js &（端口 8083 返回 503）

Phase 5  停止当前应用
  └─ pm2 stop vibe_blog_next
...
Phase 8  健康检查
  └─ curl http://127.0.0.1:8083（3 次重试，间隔 5s）
...
- `deploy-remote.sh` 用 curl 访问该接口，验证返回的 `build_commit` 与本次构建一致
- 验证通过才视为部署成功，否则自动回滚
...
├── scripts/                # 部署脚本
├── public/                 # 源静态资源（git 同步）
└── node_modules/           # 根目录依赖（webhook/maintenance 使用）
```
...
standalone 模式自带精简的 `node_modules`，服务器根目录的 `node_modules` 仅 `webhook-server.js` 和 `maintenance-server.js` 使用。如果这两个脚本的依赖变更：
