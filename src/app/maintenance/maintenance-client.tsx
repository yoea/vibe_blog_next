...
export function MaintenanceClient({ isAdmin, isMaintenanceOn }: { isAdmin: boolean; isMaintenanceOn: boolean }) {
  ...
  return (
    <div className="mt-6 space-y-3">
      <div className={`inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-medium ${
        isMaintenanceOn ? 'bg-accent text-primary' : 'bg-green-500/10 text-green-600'
      }`}>
      ...
      {isAdmin && (
        <div className="pt-2">
          ...
          >
            {loading
              ? (isMaintenanceOn ? '关闭中...' : '开启中...')
              : done
                ? (isMaintenanceOn ? '已关闭' : '已开启')
                : (isMaintenanceOn ? '关闭维护模式' : '手动进入维护模式')}
          </Button>
        </div>
      )}
